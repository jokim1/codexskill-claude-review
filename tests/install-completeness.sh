...
for helper in scripts/claude-config.sh scripts/claude-locator.sh scripts/claude-runtime.sh; do
  ...
  case "$helper" in
    ...
    *runtime*) tail -n 1 "$ROOT/$helper" | grep -Fqx '# claude-review-helper-complete: runtime_v13' || fail "$helper marker" ;;
  esac
...
printf '%s\n' "$doctor_output" | grep -Fqx 'claude_runtime_contract=direct_inherited_path_v13' || fail "doctor runtime helper bootstrap"
