...
assert_line "$basic_output" "claude_runtime_contract=direct_inherited_path_v11" "runtime contract"
...
path = Path(sys.argv[1])
marker = "# claude-review-helper-complete: runtime_v11"
...
run_bootstrap_case() {
  ...
  case "$tamper" in
    ...
    stale_runtime) sed -e 's/direct_inherited_path_v11/direct_inherited_path_v10/' -e 's/runtime_v11/runtime_v10/' "$REPO_ROOT/scripts/claude-runtime.sh" > "$skill/scripts/claude-runtime.sh" ;;
  esac
