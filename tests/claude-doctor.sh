...
assert_line "$basic_output" "claude_runtime_contract=direct_inherited_path_v10" "runtime contract"
...
path = Path(sys.argv[1])
marker = "# claude-review-helper-complete: runtime_v10"
...
run_bootstrap_case() {
  ...
  case "$tamper" in
    ...
    stale_runtime) sed -e 's/direct_inherited_path_v10/direct_inherited_path_v9/' -e 's/runtime_v10/runtime_v9/' "$REPO_ROOT/scripts/claude-runtime.sh" > "$skill/scripts/claude-runtime.sh" ;;
  esac
